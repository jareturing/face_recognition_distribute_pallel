...
from torch.utils.data import Dataset, ConcatDataset, DataLoader,BatchSampler,SequentialSampler,IterableDataset,RandomSampler
...
def get_train_loader(conf):
    if conf.data_mode in ['ms1m', 'concat']:
        ms1m_ds, ms1m_class_num = get_train_dataset(conf.ms1m_folder/'images')
        print('ms1m loader generated:class ',ms1m_class_num)
    ...
    loader = DataLoader(ds, batch_size=conf.batch_size,  shuffle=(train_sampler is None), pin_memory=conf.pin_memory,
                        num_workers=conf.num_workers,drop_last=True,sampler=train_sampler)
